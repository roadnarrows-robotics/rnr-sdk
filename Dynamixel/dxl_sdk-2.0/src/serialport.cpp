...
bool SerialPort::SetupSerialPort(int baud)
{
  ...
  if( tcgetattr(SocketFD, &newtio) == -1 )
  {
    fprintf(stderr, "tcgetattr(%d,...) failed", SocketFD);
    return false;
  }
  ...
  speed_t ospeed = baud;
  speed_t ispeed = baud;
  if( cfsetospeed(&newtio, ospeed) == -1 )   // set output baudrate in structure
  {
    fprintf(stderr, "tcsetospeed(newtio,%u)", ospeed);
    return false;
  }
  if( cfsetispeed(&newtio, ispeed) == -1 )   // set input baudrate in structure
  {
    fprintf(stderr, "tcsetispeed(newtio,%u)", ispeed);
    return false;
  }
  // RDK
