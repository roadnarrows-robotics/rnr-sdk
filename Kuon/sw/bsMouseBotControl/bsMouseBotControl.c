...
static bool_t   OptsServer      = false;        ///< executing on MouseBot
...
{
  .synopsis =
    "bsMouseBotControl is a manual control application for the\n MouseBot platforms.",

  .long_desc = 
    "The %P application is an application which allows client systems to"
    " manuever the MouseBot platforms using a standard wired Xbox360 controller,"
    ...
    " trigger on the Speed Wheel. Left trigger on all devices places the MouseBot"
...
static OptsInfo_T OptsInfo[] =
{
  ...
  // -s, --server
  {
    ...
    .opt_desc   = "This instance is executing as the daemon server on the main "
                  "MouseBot processor. The server instance has higher priority "
...
void ControlState(struct Control360 *Controller, int PercentVal){
  ...
  fprintf(stderr,"Power:%d\%\r", PercentVal);
}
...
int main(int argc, char *argv[]) {
  ...
  BsClient_P pClient = bsClientNew("mousebot");
  ...
  while(1) {
    ...
    if(hid360.Controller_Connected) {
      vConn = bsMouseBotReqOpen(pClient, "0", "1", OptsServer, false);
      if(OptsEcho){
        if(hid360.type == 1){
    ...
    while(hid360.Controller_Connected) {
      ...
      if(hid360.Center_X) {
        LOGDIAG1("\n****Exiting****\n");
        bsMouseBotReqStop(pClient, vConn);
        bsMouseBotReqClose(pClient, vConn);
      ...
      if(hid360.Left_Trig_Val > 80){
        ...
        if((hid360.Right_Trig_Val >= 70) && !coast) {
          //bsMouseBotReqAlterBrake(pClient, vConn, 0, 0, 0, 0);
          //bsMouseBotReqAlterSlew(pClient, vConn, 40, 40, 40, 40);
          coast = true;
        }
        if((hid360.Right_Trig_Val < 70) && coast) {
          //bsMouseBotReqAlterBrake(pClient, vConn, 31, 31, 31, 31);
          //bsMouseBotReqAlterSlew(pClient, vConn, 0, 0, 0, 0);
          coast = false;
        }

        usleep(10000);
        bsMouseBotReqSetMotorSpeeds(pClient, vConn,

                           GetLeftValue(hid360.Left_Y_Val,
                               hid360.Left_X_Val, PercentFull),

                           -1*GetRightValue(hid360.Left_Y_Val,
                               hid360.Left_X_Val, PercentFull));
        usleep(10000);
