...
// Package:   Laelaps
//
// Firmware:  Arduino Compatible Firmware, v1.
//
// File:      laeWatchDogV1.ino
//
/*! \file
 *
 * $LastChangedDate$
...
#define LAE_WD_FW_VERSION 1 ///< firmware version
...
int     CurBattCharge;    ///< battery charge state 0% - 100%
...
LedPattern_T LedPat[LedPatIdxNumOf] =
{
  ...
  // critical alarm pattern - slow flashing red [LedPatIdxAlarmCrit]
  { 0, 2, {1000, 1000, 0}, {{255, 0, 0}, {0, 0, 0}, {0, 0, 0}} },

  // alarm pattern - solid red [LedPatIdxAlarm]
...
void setup()
{
  ...
  CurBattCharge   = LaeWdArgBattChargeMax;
  ...
  {
    pinMode(pin, INPUT);      // must be input
...
void enterNoServiceState()
{
  ...
  updateBatteryRgb(CurBattCharge);
