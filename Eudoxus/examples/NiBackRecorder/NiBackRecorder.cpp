...
static char  *OptsModFile   = NULL;   ///< modules xml file name
static int    OptsDumpTime  = 1;      ///< cyclic buffer dump time
static bool   OptsVerbose   = false;  ///< verbose trace/debug printing
static char  *OptsMirrorSet = NULL;   ///< set sensor left/right mirroring
static bool   OptsMirrorOn  = true;   ///< set sensor left/right mirroring
static bool   OptsReg       = false;  ///< set registration of depth to image
static bool   OptsFrameSync = false;  ///< sync the image and the depth
static char  *OptsDir       = (char *)".";  ///< output directory
static bool   ArgsRecDepth  = false;  ///< record depth frames
static bool   ArgsRecImage  = false;  ///< record image frames

//
// Forward declarations.
//
static int OptsCvtArgDumpTime(const char *argv0, const char *sOptName,
                              char *optarg, void *pOptVal);
static int OptsCvtArgMirror(const char *argv0, const char *sOptName,
                              char *optarg, void *pOptVal);
static int OptsCvtArgDir(const char *argv0, const char *sOptName,
                         char *optarg, void *pOptVal);
...
static OptsPgmInfo_T AppPgmInfo =
{
  ...
  // synopsis
  "Records sensor data to file.",

  // long_desc = 
  "The %P command records frames from one or more production nodes at the "
  "specfied resolutions generated from the attached sensor.\n\n"
  "Arguments:\n"
  "   node    Sensor production node. One of: depth image\n"
  "   res     Sensor node resolution. One of: qvga  vga",
...
static OptsInfo_T AppOptsInfo[] =
{
  ...
  // -t, --dump-time <sec>
  {
    ...
    required_argument,    // has_arg
    true,                 // has_default
    &OptsDumpTime,        // opt_addr
    OptsCvtArgDumpTime,   // fn_cvt
    OptsFmtInt,           // fn_fmt
    "<sec>",              // arg_name
    "Cyclic buffers dump time (seconds).",     // opt desc
  },
  ...
  // --verbose
  {
    ...
    OptsFmtBool,          // fn_fmt
    NULL,                 // arg_name
    "Enable verbose trace/debug printing.", // opt desc
  },
  ...
  // --mirror {on | off}
  {
    ...
    required_argument,    // has_arg
    false,                // has_default
    &OptsMirrorSet,       // opt_addr
    OptsCvtArgMirror,     // fn_cvt
    OptsFmtStr,           // fn_fmt
    "{on|off}",           // arg_name
    "Set mirror properties. Horizontal flip if 'on', normal if 'off'.",
                          // opt desc
  },
  ...
  // --register
  {
    ...
    OptsFmtBool,          // fn_fmt
    NULL,                 // arg_name
    "Enable depth to image registration.", // opt desc
  },
  ...
  // --frame-sync
  {
    ...
    no_argument,          // has_arg
    true,                 // has_default
    &OptsFrameSync,       // opt_addr
    OptsCvtArgBool,       // fn_cvt
    OptsFmtBool,          // fn_fmt
    NULL,                 // arg_name
    "Enable synchronization of depth and image frames.", // opt desc
  },
  ...
  // -d, --directory <dirname>
  {
    ...
    true,                 // has_default
    &OptsDir,             // opt_addr
    OptsCvtArgDir,        // fn_cvt
    OptsFmtStr,           // fn_fmt
    "<dirname>",          // arg_name
    "Dump captured frames to oni file(s) in the given directory.", // opt desc
  },
...
static int OptsCvtArgDumpTime(const char *argv0, const char *sOptName,
                              char *optarg, void *pOptVal)
{
  // pOptVal is OptsDumpTime
  OptsCvtArgInt(argv0, sOptName, optarg, pOptVal);

  if( OptsDumpTime <= 0 )
  {
    OptsInvalid(Argv0, "'%s': Invalid '%s' dump time argument value.",
        optarg, sOptName);
  }
}
                            ...
                            char *optarg, void *pOptVal)
{
  ...
  {
    OptsInvalid(Argv0, "'%s': Invalid '%s' argument value.", optarg, sOptName);
  }
}
                         ...
                         char *optarg, void *pOptVal)
{
  ...
  if( (OptsDir == NULL) || (*OptsDir == 0) )
  {
    OptsInvalid(Argv0, "'%s': No '%s' directory argument specified.",
        optarg, sOptName);
  }
  else if( (access(OptsDir, W_OK|X_OK) != 0) )
  {
    OptsInvalid(Argv0,
        "'%s': Invalid '%s' directory argument: Cannot access.",
        optarg, sOptName);
  }
  else if( (stat(OptsDir, &statbuf) != 0) || !S_ISDIR(statbuf.st_mode) )
  {
    OptsInvalid(Argv0,
        "'%s': Invalid '%s' directory argument: Not a directory.",
        optarg, sOptName);
  }
}

static void ArgCvtRec(char *sArg, RecConfiguration &config)
{
  char  *sRec;
  char  *sRes;

  if( ((sRec = strtok(sArg, "=")) == NULL) ||
      ((sRes = strtok(NULL, "")) == NULL) )
  {
    OptsInvalid(Argv0, "'%s': Invalid '<node>=<res>' syntax.", sArg);
  }
  else if( !strcmp(sRec, "depth") )
  {
    ...
    {
      OptsInvalid(Argv0, "'%s': Unsupported '%s' resolution.", sRes, sRec);
    }
  }
  else if( !strcmp(sRec, "image") )
  {
  ...
  {
    OptsInvalid(Argv0, "'%s': Unknown/unsupported node.", sRec);
  }
}
 ...
 */
static void MainInitArgs(int argc, char *argv[], RecConfiguration &config)
{
  int   i;
  ...
  config.nDumpTime  = OptsDumpTime;
  config.bVerbose   = OptsVerbose? TRUE: FALSE;
  config.bRegister  = OptsReg? TRUE: FALSE;
  config.bFrameSync = OptsFrameSync? TRUE: FALSE;

  if( OptsMirrorSet != NULL )
  {
    config.bMirrorIndicated = TRUE;
    config.bMirror = OptsMirrorOn? TRUE: FALSE;
  }

  xnOSStrCopy(config.strDirName, OptsDir, XN_FILE_MAX_PATH);

  for(i=0; i<argc; ++i)
  {
    ArgCvtRec(argv[i], config);
  }

  if( !config.bRecordDepth && !config.bRecordImage )
  {
    OptsInvalid(Argv0, "No nodes specified: Nothing to record.\n");
  }
}
 ...
 * Program terminates on conversion error.
 */
static XnStatus MainInit(int argc, char *argv[], RecConfiguration &config)
{
  // parse, validate, and save command-line arguments
  MainInitArgs(argc, argv, config);
...
int main(int argc, char** argv)
{
  ...
  nRetVal = MainInit(argc, argv, config);
  ...
  printf("Press 'd' to dump buffers to file; Press '<ESC>' to quit.\n");
